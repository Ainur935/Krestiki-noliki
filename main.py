...
print('Добро пожаловать в игру "\033[0;31;40mКрестики\033[0;0m - \033[0;34;40mнолики\033[0;0m". Да начнется битва!\nНачинают \033[0;34;40mнолики\033[0;0m.')
beetwen_()
print(*pole)
beetwen_()
...
def enter():  # функция для ввода координат игрового символа
    ...
    if a.isalpha() or a.isspace() or len(a) > 1 or int(a) < 1:  #проверяем,не введены ли ошибочно буквы или неотображаемые символы
        # либо например в формате 06, а не 6, а также чтобы число было в пределах от 1 до 9
        print('Неверный формат (возможно присутствует пробел или 0 перед числом) или введенное число вне игрового поля')
        ...
        for x in check_matches:
            if x[0] == x[1] and x[0] == x[2]:  #в случае наличия совпадений всех символов объявляем победителя
                ...
                sys.exit()  #завершение программы в связи с победой одного из игроков
            else:  #в случае отсутствия совпадений продолжаем поиск в следующих списках
                continue
        if len(enter_symbols) > 8:  #окончание игры, если введено 9 символов (поле заполнено) и не выявлен победитель
